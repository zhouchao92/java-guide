...
是在垃圾回收算法执行过程中，需要将 JVM 内存冻结的一种状态，在 STW 状态下，所有线程都会停止（GC 线程除外），native 方法可以被执行但是不能与 JVM 进行交互。

#### 为什么垃圾回收要设计成 Stop the World 的模式？

在执行垃圾回收时，如果不暂停线程，会导致对象的状态一直发生变化，可能会有非垃圾在执行垃圾回收操作时变成垃圾，导致 GC 不准确。
